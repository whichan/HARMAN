...
void GPIO_WritePin(GPIO_TypeDef *GPIOx, uint32_t GPIO_Pin, gpio_level_t level) //특정 핀 하나 제어
{
   if(level == HIGH) {
...
void GPIO_WritePort(GPIO_TypeDef *GPIOx, uint32_t data) //포트 내의 전체 핀 제어
{
      GPIOx->ODR = data;
}
