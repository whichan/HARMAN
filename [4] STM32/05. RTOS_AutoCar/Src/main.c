...
extern volatile uint8_t auto_mode_state; //dcmotor.c에 있는 변수 연결
...
// LCD 관련 함수들도 extern 선언이 필요할 수 있습니다.
...
int main(void)
{
 ...
 HAL_TIM_PWM_Start_IT(&htim3, TIM_CHANNEL_1);  // DC motor PWM
 HAL_TIM_PWM_Start_IT(&htim3, TIM_CHANNEL_2);  // DC motor PWM
...
void StartTask01(void *argument)
{
 ...
 for(;;)
 {
   pc_command_processing(); // PC 명령 처리는 항상 유지 (디버깅용)

   if (auto_mode_state)
   {
       auto_drive();       // 자율주행 모드 실행
   }
   else
   {
       manual_mode_run();   // 수동 모드 실행
   }

   osDelay(10); // 다른 태스크에 CPU 양보
 }
