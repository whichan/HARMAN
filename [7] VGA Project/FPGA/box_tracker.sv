...
module box_tracker #(
    parameter NUM_BOXES   = 2,
    parameter DIST_THRESH = 40,  // 미사용 (호환성 유지)
    parameter HOLD_MAX    = 10
) (
    input logic clk,
    input logic reset,
  ...
  // ── 임시변수 ──────────────────────────────────────────────────
  logic [8:0] raw_cx[0:NUM_BOXES-1];
  logic slot_updated[0:NUM_BOXES-1];  // 이번 프레임에 슬롯 업데이트 됐는지
  ...
  always_ff @(posedge clk) begin
    if (reset) begin
      ...
      // Step 1: slot_updated 초기화
      for (i = 0; i < NUM_BOXES; i++) begin
        slot_updated[i] = 1'b0;
        raw_cx[i] = (raw_x_min[i] + raw_x_max[i]) >> 1;
      end

      // Step 2: x좌표 기준으로 슬롯 강제 할당
      //   slot[0] = center_x < 160  (왼쪽 객체)
      //   slot[1] = center_x >= 160 (오른쪽 객체)
      for (i = 0; i < NUM_BOXES; i++) begin
        if (raw_valid[i]) begin
          if (raw_cx[i] < 9'd160) begin
            // 왼쪽 → slot[0]
            trk_x_min[0] <= raw_x_min[i];
            trk_x_max[0] <= raw_x_max[i];
            trk_y_min[0] <= raw_y_min[i];
            trk_y_max[0] <= raw_y_max[i];
            trk_valid[0] <= 1'b1;
            hold_cnt[0]  <= HOLD_MAX[$clog2(HOLD_MAX+1):0];
            slot_updated[0] = 1'b1;
          end else begin
            // 오른쪽 → slot[1]
            trk_x_min[1] <= raw_x_min[i];
            trk_x_max[1] <= raw_x_max[i];
            trk_y_min[1] <= raw_y_min[i];
            trk_y_max[1] <= raw_y_max[i];
            trk_valid[1] <= 1'b1;
            hold_cnt[1]  <= HOLD_MAX[$clog2(HOLD_MAX+1):0];
            slot_updated[1] = 1'b1;
          end
        end
      end

      // Step 3: 업데이트 안 된 슬롯 → hold_cnt 감소
      for (i = 0; i < NUM_BOXES; i++) begin
        if (!slot_updated[i]) begin
          if (hold_cnt[i] > 0) begin
            hold_cnt[i] <= hold_cnt[i] - 1;
            if (hold_cnt[i] == 1) trk_valid[i] <= 1'b0;
          end
        end
      end
